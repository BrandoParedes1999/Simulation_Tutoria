    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4" style="min-height:520px"
        ...
        <div class="bg-white rounded-2xl border border-blue-100 shadow-sm flex flex-col overflow-hidden">
            ...
            <div class="flex border-b border-blue-50 p-2 gap-1">
                <button
                    @click="tab = 'recibidos'"
                    :class="tab === 'recibidos' ? 'bg-blue-700 text-white' : 'text-blue-500 hover:bg-blue-50'"
                    class="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 text-xs font-medium rounded-xl transition-colors">
                    Recibidos
                    @if($noLeidos > 0)
                        <span :class="tab === 'recibidos' ? 'bg-white text-blue-700' : 'bg-blue-600 text-white'"
                ...
                <button
                    @click="tab = 'enviados'"
                    :class="tab === 'enviados' ? 'bg-blue-700 text-white' : 'text-blue-500 hover:bg-blue-50'"
                    class="flex-1 px-2 py-1.5 text-xs font-medium rounded-xl transition-colors">
                    Enviados
                </button>
                <button
                    @click="tab = 'urgentes'"
                    :class="tab === 'urgentes' ? 'bg-red-600 text-white' : 'text-blue-500 hover:bg-blue-50'"
                    class="flex-1 px-2 py-1.5 text-xs font-medium rounded-xl transition-colors">
                    Urgentes
                </button>
            </div>
            ...
            <div class="flex-1 overflow-y-auto divide-y divide-blue-50">
                ...
                {{-- Recibidos --}}
                <div x-show="tab === 'recibidos'">
                    @forelse($recibidos as $conv)
                        @php
                            $contraparte = $conv->remitente->name ?? '—';
                            $noLeido     = !$conv->leido_en;
                        @endphp
                        <button
                            ...
                            <div class="flex items-start gap-2.5">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <p class="text-xs font-semibold text-blue-900 truncate {{ $noLeido ? 'font-bold' : '' }}">{{ $contraparte }}</p>
                                        <span class="text-[10px] text-blue-400 flex-shrink-0 ml-1">{{ $conv->created_at->locale('es')->isoFormat('D MMM') }}</span>
                                    </div>
                                    <p class="text-xs text-blue-700 truncate mt-0.5 {{ $noLeido ? 'font-semibold' : '' }}">{{ $conv->asunto }}</p>
                    ...
                    @empty
                        <div class="flex flex-col items-center justify-center py-10">
                            ...
                            <p class="text-xs text-blue-400">Sin mensajes recibidos</p>
                        </div>
                    @endforelse
                </div>

                {{-- Enviados --}}
                <div x-show="tab === 'enviados'">
                    @forelse($enviados as $conv)
                        @php $contraparte = $conv->destinatario->name ?? '—'; @endphp
                        <button
                            wire:click="seleccionar({{ $conv->id }})"
                            class="w-full text-left p-3 transition-colors hover:bg-blue-50/50
                                {{ $conversacionActivaId === $conv->id ? 'bg-blue-50 border-l-2 border-blue-600' : '' }}">
                            <div class="flex items-start gap-2.5">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <p class="text-xs font-semibold text-blue-900 truncate">{{ $contraparte }}</p>
                                        <span class="text-[10px] text-blue-400 flex-shrink-0 ml-1">{{ $conv->created_at->locale('es')->isoFormat('D MMM') }}</span>
                                    </div>
                                    <p class="text-xs text-blue-700 truncate mt-0.5">{{ $conv->asunto }}</p>
                                    <div class="flex items-center gap-2 mt-1">
                                        @if($conv->prioridad === 'urgente')
                                            <span class="text-[10px] px-1.5 py-0.5 bg-red-100 text-red-600 rounded-full font-medium">Urgente</span>
                                        @endif
                    ...
                    @empty
                        <div class="flex flex-col items-center justify-center py-10">
                            @svg('lucide-inbox', 'w-8 h-8 text-blue-200 mb-2')
                            <p class="text-xs text-blue-400">Sin mensajes enviados</p>
                        </div>
                    @endforelse
                </div>

                {{-- Urgentes --}}
                <div x-show="tab === 'urgentes'">
                        ...
                        <button
                            ...
                            <div class="flex items-start gap-2.5">
                                <div class="w-9 h-9 bg-red-100 rounded-full flex items-center justify-center flex-shrink-0">
                                    <span class="text-red-700 font-bold text-xs">{{ strtoupper(substr($contraparte, 0, 1)) }}</span>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <p class="text-xs font-semibold text-blue-900 truncate">{{ $contraparte }}</p>
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-[10px] px-1.5 py-0.5 bg-red-100 text-red-600 rounded-full font-medium">Urgente</span>
                                        @if($noLeido)<span class="w-2 h-2 bg-blue-500 rounded-full"></span>@endif
                                        <span class="text-[10px] text-blue-400">{{ $conv->respuestas->count() }} respuestas</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    @empty
                        <div class="flex flex-col items-center justify-center py-10">
                            @svg('lucide-inbox', 'w-8 h-8 text-blue-200 mb-2')
                            <p class="text-xs text-blue-400">Sin mensajes urgentes</p>
                        </div>
                    @endforelse
                </div>
                    ...
                    <div class="{{ $esMioRaiz ? 'flex justify-end' : 'flex justify-start' }}">
                        <div class="max-w-xs lg:max-w-md">
                            <div class="{{ $esMioRaiz ? 'bg-blue-600 text-white rounded-2xl rounded-tr-sm' : 'bg-slate-100 text-slate-800 rounded-2xl rounded-tl-sm' }} px-4 py-3">
                                <p class="text-xs {{ $esMioRaiz ? 'text-blue-200' : 'text-blue-500' }} font-semibold mb-1">{{ $conversacionActiva->asunto }}</p>
                                <p class="text-sm leading-relaxed">{{ $conversacionActiva->contenido }}</p>
                            </div>
                            <p class="text-xs text-slate-400 {{ $esMioRaiz ? 'text-right' : '' }} mt-1">{{ $conversacionActiva->created_at->locale('es')->isoFormat('D MMM, H:mm') }}</p>
                        </div>
                    </div>
                    ...
                    @foreach($conversacionActiva->respuestas as $resp)
                        ...
                        <div class="{{ $esMioResp ? 'flex justify-end' : 'flex justify-start' }}">
                            <div class="max-w-xs lg:max-w-md">
                                ...
                                <p class="text-xs text-slate-400 {{ $esMioResp ? 'text-right' : '' }} mt-1">{{ $resp->created_at->locale('es')->isoFormat('D MMM, H:mm') }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
