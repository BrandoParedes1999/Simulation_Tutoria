<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
    ...
    <div class="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden"
         ...
         @mudar-tab.window="tab = $event.detail.tab">

        <div class="flex border-b border-blue-100">
            <button
                @click="tab = 'disponibles'"
                :class="tab === 'disponibles' ? 'text-blue-700 bg-blue-50/50' : 'text-blue-400 hover:text-blue-600 hover:bg-blue-50/30'"
                ...
                <span :class="tab === 'disponibles' ? 'bg-blue-700 text-white' : 'bg-blue-100 text-blue-600'"
                      class="inline-flex items-center justify-center px-1.5 min-w-[20px] h-5 text-[10px] font-bold rounded-full">
                    {{ $disponibles->count() }}
                </span>
                <span x-show="tab === 'disponibles'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-700"></span>
            </button>

            <button
                @click="tab = 'carrito'"
                :class="tab === 'carrito' ? 'text-blue-700 bg-blue-50/50' : 'text-blue-400 hover:text-blue-600 hover:bg-blue-50/30'"
                class="flex-1 flex items-center justify-center gap-2 px-3 py-3 text-sm font-medium transition-colors relative">
                @svg('lucide-shopping-cart', 'w-4 h-4')
                <span>Carrito</span>
                @if(count($carrito) > 0)
                    <span :class="tab === 'carrito' ? 'bg-amber-500 text-white' : 'bg-amber-100 text-amber-700'"
                          class="inline-flex items-center justify-center px-1.5 min-w-[20px] h-5 text-[10px] font-bold rounded-full">
                        {{ count($carrito) }}
                    </span>
                @endif
                <span x-show="tab === 'carrito'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-700"></span>
            </button>

            <button
                @click="tab = 'inscritas'"
                :class="tab === 'inscritas' ? 'text-blue-700 bg-blue-50/50' : 'text-blue-400 hover:text-blue-600 hover:bg-blue-50/30'"
                class="flex-1 flex items-center justify-center gap-2 px-3 py-3 text-sm font-medium transition-colors relative">
                @svg('lucide-check-circle-2', 'w-4 h-4')
                <span>Inscritas</span>
                <span :class="tab === 'inscritas' ? 'bg-emerald-500 text-white' : 'bg-emerald-100 text-emerald-700'"
                      class="inline-flex items-center justify-center px-1.5 min-w-[20px] h-5 text-[10px] font-bold rounded-full">
                    {{ $inscritas->count() }}
                </span>
                <span x-show="tab === 'inscritas'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-700"></span>
            </button>
        </div>
        ...
        <div class="p-4 sm:p-5">
            ...
            {{-- TAB: DISPONIBLES --}}
            <div x-show="tab === 'disponibles'" class="space-y-4">
            ...
            {{-- TAB: CARRITO --}}
            <div x-show="tab === 'carrito'" class="space-y-4">
            ...
            {{-- TAB: INSCRITAS --}}
            <div x-show="tab === 'inscritas'" class="space-y-3">
