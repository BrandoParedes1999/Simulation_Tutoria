<x-app-layout>
    @php
        ...
        // Alertas por alumno (para badges)
        ...
        // FIX #16: umbral en escala 0-100 → < 75 (no < 7.5)
        ...
        $conversaciones = \App\Models\Mensaje::where(function ($q) use ($userId) {
            ...
            ->with(['remitente:id,name', 'destinatario:id,name', 'respuestas.remitente:id,name'])
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5"
         x-data="{
            ...
            urlEnviar:   '{{ route('tutor.mensajes.enviar') }}',
            ...
            enviarMensaje() {
                ...
                if (this.tipoDestinatario === 'alumno' && !this.destinatarioId) return;
                const body = {
                ...
                .then(data => {
                    if (data.ok) {
                        ...
                        this.asunto = ''; this.contenido = ''; this.prioridad = 'normal';
                        this.destinatarioId = null; this.plantillaSeleccionada = '';
                        window.location.reload();
                    } else {
                        alert('Error al enviar: ' + (data.error || 'desconocido'));
                    }
                })
                .catch(() => alert('Error de conexión al enviar el mensaje'));
            },
        ...
        {{-- Banner alumnos críticos (< 75 pts) --}}
                        ...
                        <div class="flex items-center gap-2 bg-white border border-red-100 rounded-xl px-3 py-2">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-red-500">
                                    Prom. {{ number_format((float)$ac->promedio_general, 1) }} pts
                                </p>
                            </div>
            ...
            <div class="bg-white rounded-2xl border border-blue-100 shadow-sm flex flex-col overflow-hidden">
                ...
                <div class="p-3 border-b border-blue-50">
                    <div class="relative">
                        ...
                        <input x-model="busqueda" type="text" placeholder="Buscar conversación..."
                ...
                <div class="flex border-b border-blue-50 px-3 pt-2 gap-1">
                    <button @click="pestana = 'recibidos'"
                            :class="pestana === 'recibidos' ? 'bg-blue-600 text-white' : 'text-blue-500 hover:bg-blue-50'"
                            class="px-3 py-1 text-xs font-medium rounded-lg transition flex items-center gap-1">
                        Recibidos
                        @if($noLeidos > 0)
                            <span class="bg-white text-blue-600 text-xs font-bold px-1.5 rounded-full">{{ $noLeidos }}</span>
                        @endif
                    </button>
                    <button @click="pestana = 'enviados'"
                            :class="pestana === 'enviados' ? 'bg-blue-600 text-white' : 'text-blue-500 hover:bg-blue-50'"
                            class="px-3 py-1 text-xs font-medium rounded-lg transition">
                        Enviados
                    </button>
                    <button @click="pestana = 'urgentes'"
                            :class="pestana === 'urgentes' ? 'bg-blue-600 text-white' : 'text-blue-500 hover:bg-blue-50'"
                            class="px-3 py-1 text-xs font-medium rounded-lg transition flex items-center gap-1">
                        Urgentes
                        @if($urgentes > 0)
                            <span class="bg-red-500 text-white text-xs font-bold px-1.5 rounded-full">{{ $urgentes }}</span>
                        @endif
                    </button>
                </div>
                            ...
                            <div class="flex items-start gap-2.5">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-1 mt-1">
                                        ...
                                        <span class="text-xs text-slate-400" x-text="conv.respuestas.length + ' mensajes'"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="flex justify-start">
                                    <div class="max-w-xs lg:max-w-md">
                                        <div class="bg-slate-100 rounded-2xl rounded-tl-sm px-4 py-3">
                                            <p class="text-xs font-semibold text-blue-700 mb-1" x-text="conversacionActiva.asunto"></p>
                                            <p class="text-sm text-slate-700 leading-relaxed" x-text="conversacionActiva.contenido"></p>
                                        </div>
                                        <p class="text-xs text-slate-400 mt-1" x-text="conversacionActiva.fecha"></p>
                                    </div>
                                </div>
                            </template>
                                ...
                                <div :class="resp.remitente_id === userId ? 'flex justify-end' : 'flex justify-start'">
                                    <div class="max-w-xs lg:max-w-md">
                                        <div :class="resp.remitente_id === userId
                                            ...
                                            <p :class="resp.remitente_id === userId ? 'text-sm leading-relaxed' : 'text-sm text-slate-700 leading-relaxed'"
                                               x-text="resp.contenido"></p>
                                        </div>
                                        <p :class="resp.remitente_id === userId ? 'text-xs text-slate-400 text-right mt-1' : 'text-xs text-slate-400 mt-1'"
                                           x-text="resp.fecha"></p>
                                    </div>
                                </div>
                            </template>
        ...
        <div x-show="modalAbierto"
            ...
            <div @click.outside="modalAbierto = false"
                 class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-screen overflow-y-auto">
                ...
                <div class="p-5 space-y-4">
                            ...
                            @foreach($plantillas as $p)
                                <button @click="usarPlantilla('{{ $p['clave'] }}', '{{ $p['label'] }}')"
                                        :class="plantillaSeleccionada === '{{ $p['clave'] }}'
                                            ? 'border-blue-400 bg-blue-50' : 'border-slate-200 hover:border-blue-300'"
                                ...
                                <template x-for="a in alumnos" :key="a.usuario_id">
                                    <div @click="destinatarioId = a.usuario_id"
                                         :class="destinatarioId === a.usuario_id
                                             ? 'border-blue-400 bg-blue-50' : 'border-slate-100 hover:border-blue-200'"
                                         class="flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition">
                                        <div class="w-9 h-9 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0">
                                            <span class="text-blue-700 font-bold text-xs" x-text="a.nombre.charAt(0).toUpperCase()"></span>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-semibold text-blue-900" x-text="a.nombre"></p>
                                            {{-- Promedio en escala 0-100 --}}
                                            <p class="text-xs text-blue-400" x-text="a.carrera + ' · Prom. ' + a.promedio + ' pts'"></p>
                                        </div>
                                        <template x-if="a.alertas > 0">
                                            <span class="px-2 py-0.5 bg-red-100 text-red-600 text-xs font-bold rounded-full"
                                                  x-text="a.alertas + (a.alertas === 1 ? ' alerta' : ' alertas')"></span>
                                        </template>
                                    </div>
                                </template>
                            </div>
                        ...
                        <div class="flex gap-2">
                            @foreach(['urgente' => 'Urgente', 'normal' => 'Normal', 'informativo' => 'Informativo'] as $val => $lbl)
                                <button @click="prioridad = '{{ $val }}'"
                                        ...
                                        class="flex items-center gap-1.5 px-3 py-2 border rounded-xl text-xs transition">
                                    {{ $lbl }}
                                </button>
                            @endforeach
                        </div>
                    ...
                    <div>
                        ...
                        <textarea x-model="contenido" rows="4" placeholder="Escribe tu mensaje aquí..."
                                  class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm
