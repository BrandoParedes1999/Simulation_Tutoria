...
<body class="font-sans antialiased bg-[#f0f7ff]">
    <div class="min-h-screen">
        <x-app-header />
        @isset($header)
            <header class="bg-white shadow-sm border-b border-blue-100 hidden lg:block">
                <div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">{{ $header }}</div>
            </header>
        @endisset
        <main class="pb-20 lg:pb-0">{{ $slot }}</main>
        <x-mobile-nav />
    </div>
    @livewireScripts
    <div x-data="{
            toasts:[],
            add(tipo,mensaje){ const id=Date.now(); this.toasts.push({id,tipo,mensaje}); setTimeout(()=>this.toasts=this.toasts.filter(t=>t.id!==id),3500); }
         }"
